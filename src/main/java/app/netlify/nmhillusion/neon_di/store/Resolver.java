...
public class Resolver {
	...
	private <T> T findNeonInstanceWithSmallestNeon(Class<T> classToFind, List<NeonModel> foundList) {
		...
		final List<NeonModel> smallestModels = new ArrayList<>();

		for (NeonModel model : foundList) {
			final Neon neon = model.getOwnAnnotation();
			if (neon.priority() < smallestPriority) {
				smallestPriority = neon.priority();

				smallestModels.clear();
				smallestModels.add(model);
			} else if (neon.priority() == smallestPriority) {
				smallestModels.add(model);
			}
		}

		if (1 < smallestModels.size()) {
			throw new RuntimeException("Found more than one neon with the smallest same priority [" + smallestPriority + "]" +
